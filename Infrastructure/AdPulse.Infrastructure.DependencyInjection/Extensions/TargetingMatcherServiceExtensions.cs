...
using AdPulse.Infrastructure.DependencyInjection.Configuration;
using AdPulse.Infrastructure.DependencyInjection.Validation;
using AdPulse.Infrastructure.DependencyInjection.Services;
using AdPulse.Infrastructure.DependencyInjection.HealthChecks;
using System.Reflection;

namespace AdPulse.Infrastructure.DependencyInjection.Extensions;
...
public static class TargetingMatcherServiceExtensions
{
    ...
    private static void RegisterFSharpMatchers(IServiceCollection services, TargetingMatcherOptions options)
    {
        ...
        var fsharpAssembly = AppDomain.CurrentDomain.GetAssemblies()
            .FirstOrDefault(a => a.GetName().Name == "AdPulse.Strategies.Targeting");
        ...
        if (fsharpAssembly == null)
        {
            ...
            try
            {
                fsharpAssembly = Assembly.LoadFrom("AdPulse.Strategies.Targeting.dll");
            }
            catch
            {
                // 程序集加载失败，跳过F#匹配器注册
        ...
        {
            { "demographic", "AdPulse.Strategies.Targeting.Matchers.DemographicTargetingMatcher" },
            { "geolocation", "AdPulse.Strategies.Targeting.Matchers.GeoLocationTargetingMatcher" },
            { "time", "AdPulse.Strategies.Targeting.Matchers.TimeTargetingMatcher" },
            { "device", "AdPulse.Strategies.Targeting.Matchers.DeviceTargetingMatcher" },
            { "user-interest", "AdPulse.Strategies.Targeting.Matchers.UserInterestTargetingMatcher" },
            { "user-behavior", "AdPulse.Strategies.Targeting.Matchers.UserBehaviorTargetingMatcher" },
            { "user-preference", "AdPulse.Strategies.Targeting.Matchers.UserPreferenceTargetingMatcher" },
            { "user-tag", "AdPulse.Strategies.Targeting.Matchers.UserTagTargetingMatcher" },
            { "user-value", "AdPulse.Strategies.Targeting.Matchers.UserValueTargetingMatcher" }
        };
