namespace AdPulse.Strategies.Targeting.Utils

open System
open System.Collections.Generic
open AdPulse.Core.Domain.ValueObjects
open AdPulse.Core.Shared.Enums
...
module DataValidation =
    ...
    let validateEducation (educationValue: string) : bool * string option =
        if String.IsNullOrWhiteSpace(educationValue) then
            ...
            let mutable result = AdPulse.Core.Shared.Enums.EducationLevel.Unknown

            if Enum.TryParse<AdPulse.Core.Shared.Enums.EducationLevel>(educationValue, true, &result) then
                (true, None)
            else
    ...
    let validateOccupation (occupationValue: string) : bool * string option =
        if String.IsNullOrWhiteSpace(occupationValue) then
            ...
            let mutable result = AdPulse.Core.Shared.Enums.OccupationType.Unknown

            if Enum.TryParse<AdPulse.Core.Shared.Enums.OccupationType>(occupationValue, true, &result) then
                (true, None)
            else
    ...
    let validateIncome (incomeValue: string) : bool * string option =
        if String.IsNullOrWhiteSpace(incomeValue) then
            ...
            let mutable result = AdPulse.Core.Shared.Enums.IncomeLevel.Unknown

            if Enum.TryParse<AdPulse.Core.Shared.Enums.IncomeLevel>(incomeValue, true, &result) then
                (true, None)
            else
    ...
    let validateMaritalStatus (maritalValue: string) : bool * string option =
        if String.IsNullOrWhiteSpace(maritalValue) then
            ...
            let mutable result = AdPulse.Core.Shared.Enums.MaritalStatus.Unknown

            if Enum.TryParse<AdPulse.Core.Shared.Enums.MaritalStatus>(maritalValue, true, &result) then
                (true, None)
            else
