...
namespace AdPulse.Strategies.Targeting.Tests.Matchers
...
module DemographicMatcherTests =
    ...
    let ``DemographicTargetingMatcher应该正确实现ITargetingMatcher接口`` () =
        ...
        let imatcher =
            matcher :> AdPulse.Core.AdEngine.Abstractions.Interfaces.ITargetingMatcher
    ...
    let ``IsSupported应该正确识别支持的定向条件类型`` () =
        // Arrange
        let matcher =
            DemographicTargetingMatcher() :> AdPulse.Core.AdEngine.Abstractions.Interfaces.ITargetingMatcher
